...
birdpage.messages = function(opt_data, opt_ignored, opt_ijData) {
  return apps.messages(null, null, opt_ijData) + '<div style="display: none"><span id="Bird_hungry">hungry</span><span id="Bird_heading">heading</span><span id="Bird_if">if</span><span id="Bird_doCode">do</span><span id="Bird_elseCode">else</span><span id="Bird_hungryTooltip">The condition when the bird has not gotten the \\nfood. </span><span id="Bird_headingTooltip">Move in the direction of the given angle: 0 is \\nto the right, 90 is straight up, etc. </span><span id="Bird_positionTooltip">x and y mark the bird\'s position. \\nWhen x = 0 the bird is near the left \\nedge, when x = 100 it\'s near the \\nright edge. When y = 0 the bird is \\nat the bottom, when y = 100 it\'s at \\nthe top. </span><span id="Bird_nextLevel">Congratulations! Are you ready to proceed to level %1?</span><span id="Bird_finalLevel">Congratulations! You have solved the final level.</span></div>';
};
...
birdpage.start = function(opt_data, opt_ignored, opt_ijData) {
  ...
  for (var i176 = 1; i176 < 11; i176++) {
    output += ' ' + ((i176 == opt_ijData.level) ? '<span class="tab" id="selected">' + soy.$$escapeHtml(i176) + '</span>' : (i176 < opt_ijData.level) ? '<a class="tab previous" href="?lang=' + soy.$$escapeHtml(opt_ijData.lang) + '&level=' + soy.$$escapeHtml(i176) + '">' + soy.$$escapeHtml(i176) + '</a>' : '<a class="tab" href="?lang=' + soy.$$escapeHtml(opt_ijData.lang) + '&level=' + soy.$$escapeHtml(i176) + '">' + soy.$$escapeHtml(i176) + '</a>');
  }
